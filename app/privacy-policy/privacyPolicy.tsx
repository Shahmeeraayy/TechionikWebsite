...
import {
  PrivacyPolicyData,
  PrivacyPolicyDataItem,
} from "@/data/privacyPolicyData";

interface PrivacyPolicyProps {
  data: PrivacyPolicyData;
  innerData: PrivacyPolicyDataItem[];
}

const PrivacyPolicy = ({ data, innerData }: PrivacyPolicyProps) => {
  ...
  const handleLinkClick = (id: string) => {
    ...
    if (element) {
      const offset = 120; // Matches blog offset
                ...
                return (
                  <a
                    ...
                    className={`transition-all border-l-2 py-3 pl-4 scroll-smooth duration-100 text-sm ${
                      isActive
                        ? "text-gradient-dark border-[#F05323] font-medium"
