      ...
      <div className="flex flex-col space-y-10 max-w-8xl mx-auto md:px-10 px-4 lg:px-20">
        ...
        <div className="overflow-hidden" ref={emblaRef}>
          <div className="flex gap-4 px-4 md:px-6 lg:px-8 2xl:px-2">
            {(Array.isArray(data?.cards) ? data.cards : []).map(
              (item, index) => (
                <div
                  key={index}
                  className="
                        flex-shrink-0 px-4 rounded-lg flex flex-col cursor-pointer
                        w-[260px] min-h-[420px]
                        sm:w-[300px] sm:min-h-[460px]
                        md:w-[340px] md:min-h-[522px]
                      "
                  style={{
                    background:
                      activeIndex === index
                  ...
                  onClick={() => handleTabClick(index)}
                >
                  {/* Big Number Section with fixed height for alignment */}
                  ...
                  <div className="pb-10 space-y-4 flex flex-col flex-grow">
                    <h3 className="text-xl md:text-2xl font-bold min-h-[3.5rem] flex items-start">
                      {item.title}
                    </h3>
                      ...
                      {Array.isArray(item.description) ? (
                        <ul className="space-y-2">
                          {item.description.map((desc: string, i: number) => (
                            <li
                              key={i}
                              className="text-sm text-muted opacity-90"
                            >
                              {desc}
                            </li>
                          ))}
                        ...
                        <div
                          className="space-y-1 text-sm md:text-base text-white opacity-90"
