...
const MegaMenu = ({ data, parentSlug, isOpen, onClose }: MegaMenuProps) => {
  ...
  return (
    <div
      className={`absolute left-0 right-0 top-13 pt-4 z-50 transition-all duration-300 ease-out ${isOpen ? "opacity-100 visible translate-y-0 pointer-events-auto" : "opacity-0 invisible pointer-events-none"}`}
    >
      <div className="max-w-full mx-auto bg-white shadow-2xl overflow-hidden flex h-[650px] border border-white/10 ring-1 ring-black/5">
        ...
        <div className="lg:w-[25%] xl:w-[20%] bg-[#51555D] flex flex-col h-[100%] max-h-full relative z-20">
          {/* Scrollable List */}
          <div className="flex-1 overflow-y-auto pt-6 mb-7">
          ...
          <div className="shrink-0">
            ...
            <LoadingLink
              onClick={handleLinkClick}
              href={`/${parentSlug}`}
              className={`${parentSlug && "flex"} mx-4 mb-4 items-center justify-between px-5 py-3.5 rounded-xl border-2 border-white bg-white/5 hover:bg-white/10 transition-all duration-300`}
            >
              <span className="text-white/70 hover:text-white text-[15px] font-medium transition-colors">
                View All{" "}
            ...
            {/* Certificates */}
            <div className="flex gap-3 justify-start px-4 pb-4 opacity-70">
              <Image
        ...
        <div className="lg:w-[25%] xl:w-[20%] bg-[#F5F5F7] p-8 flex flex-col shrink-0 border-l border-gray-200">
          <h4 className="text-gray-500 font-bold text-sm mb-6 uppercase tracking-wider">
            Featured
          </h4>
