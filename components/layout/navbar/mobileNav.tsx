...
const MobileMenu: React.FC<MobileMenuProps> = ({ navLinks }) => {
  ...
  const slideBack = () => {
    setAnimating(true);
    setTimeout(() => {
      // if we're in a sub panel, go back to mega panel
      if (panel?.type === "sub") {
        // find the mega panel that owns this sub
        const ownerLink = navLinks.find(
          (l) =>
            l.isMega &&
        ...
        if (ownerLink) {
          setPanel({
            type: "mega",
            slug: ownerLink.name.toLowerCase(),
            label: ownerLink.name,
            items: ownerLink.children as MenuItem[],
            baseHref: ownerLink.href || "",
          });
        } else {
          setPanel(null);
        }
  ...
  return (
    <div className="lg:hidden flex items-center" ref={menuRef}>
      ...
      >
        <span className={`block h-[2px] bg-white rounded-full transition-all duration-300 ${isOpen ? "w-6 rotate-45 translate-y-[7px]" : "w-6"}`} />
        <span className={`block h-[2px] bg-white rounded-full transition-all duration-300 ${isOpen ? "w-0 opacity-0" : "w-4 group-hover:w-6"}`} />
        <span className={`block h-[2px] bg-white rounded-full transition-all duration-300 ${isOpen ? "w-6 -rotate-45 -translate-y-[7px]" : "w-6"}`} />
      </button>
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-white/[0.06] shrink-0">
          {panel ? (
            ...
            <LoadingLink href="/" onClick={() => handleClose()}>
              <Image
                src="/Logo.webp"
                alt="Logo"
                width={130}
        ...
        >
          ...
          {!panel && (
            <nav className="py-4 px-4 flex flex-col gap-1">
              {navLinks.map((link) => {
                if (link.isMega) {
                  const items = link.children as MenuItem[];
                  return (
                    <div key={link.name} className="flex items-center rounded-lg hover:bg-white/[0.04] transition-all">
