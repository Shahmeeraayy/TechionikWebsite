  ...
  return (
    <main>
      <div className="flex flex-col gap-2 mb-10">
        <h2 className="w-full lg:w-[70%] 2xl:w-[40%]">
          <span className="text-dark-gradient">{whatwedo.title}</span>
        </h2>
        <div
          className="text-muted w-full lg:w-[50%]"
          dangerouslySetInnerHTML={createMarkup(whatwedo.descriptiption)}
        />
        <p className="text-muted">{whatwedo.mainDescription}</p>
      </div>
        ...
        <div className="flex gap-6">
          {whatwedo.card?.map((item, index) => (
            ...
            >
              ...
              <div
                className="flex flex-col p-4 gap-4 w-full lg:w-[80%] text-muted"
